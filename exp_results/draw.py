...
for method in METHODS:
    ...
    # 处理locat
    qcsa_json_file = json_file + '.before_qcsa'
    if os.path.exists(qcsa_json_file):
        before_qcsa_data = json.load(open(qcsa_json_file))
        # 替换掉data['observations']中before_qcsa_data中有的轮次的observation
        replace_observations = before_qcsa_data['observations']
        data["observations"][:len(replace_observations)] = replace_observations
